...
\begin{abstract}
% Systems biology has experienced dramatic growth in the number, size and complexity of computational models describing biology. To reproduce simulation results and reuse models, researchers need to exchange precise and unambiguous descriptions of model structure and meaning. SBML (the Systems Biology Markup Language) is a community-developed format for this purpose. The latest edition, called SBML Level~3, has a modular structure, with a core suited to representing reaction-based models, and packages that extend the core with features suited for a variety of model types. Examples include constraint-based models, reaction-diffusion models, logical network models, and rule-based models. SBML and its rich software ecosystem have transformed the way systems biologists build and interact with models, and has played an important role in increasing model interoperability and reuse over the past two decades. More recently, the rise of multiscale models of whole cells and organs, and new data sources such as single cells measurements and live imaging, have precipitated new ways of integrating data and models. SBML Level~3 provides the foundation needed to support this evolution.

Systems biology has experienced dramatic growth in the number, size and complexity of computational models describing biology. This review article provides an overview of SBML Level~3 (the current edition of SBML) and it’s development over the last decade to further expand the scope of models that can be encoded in a precise, unambiguous format. The modular approach adopted by SBML~L3 provides  a core suited to representing reaction-based models, and packages that extend the core with features suited for a variety of model types. Examples include constraint-based models, reaction-diffusion models, logical network models, and rule-based models. A community developed standard, SBML and its rich software ecosystem have transformed the way systems biologists build and interact with models, and has played an important role in increasing model interoperability and reuse over the past two decades. More recently, the rise of multiscale models of whole cells and organs, and new data sources such as single cells measurements and live imaging, have precipitated new ways of integrating data and models. The review concludes with a discussion of the challenges presented by these developments and how SBML Level~3 provides the foundation needed to support this evolution.
...
In this article, we describe the evolution of SBML Level~3 over the last decade. Firstly, we outline the structure of SBML, then detail the modularity of L3 and the additional modeling scenarios facilitated by the range of L3 packages. We then discuss the community aspect  of SBML development and outline the impact SBML has had on the community directly involved as well as on the wider scientific arena. Lastly, we reflect on issues that have been identified as challenges for the continued development of the standard.


\begin{figure}[b]
  \center
  \includegraphics[width=\textwidth]{resources/SBML-Level3-v11.pdf}
...
The SBML community strives to facilitate the standard adoption and evolution. This led to introducing modularity in SBML L3 which facilitated defining the SBML features described in the next section.
\clearpage
...
Though this modular approach has benefits, it is not without potential pitfalls.  The main risks are fragmentation of the community, and incompatibility of packages due to complex feature dependencies.  The SBML community addressed the former by maintaining communications between package developers.  The community structuring described below aims to enable such interactions. As for the latter, API libraries (see Box~\ref{box:software}) can handle \emph{some} combinations of packages and hide some of the complexity.  Still, there remain some combinations of packages that are not fully understood, and it remains for future work to define how (if ever) they can be combined for use in a single model.
...
%HERE we need add the issue of journals and funding agencies]
A wider impact of SBML as a \emph{de facto} standard has been the support of publishers and funding agencies. 
The majority of funding agencies have started to publish policies related to data-sharing and data-management, which cover research producing computational models.
For example, the SEEK platform for model management has become an integral part of the scientific landscape - and it uses SBML as one of their main formats for model storage and simulation  \citep{stanford2019data}.
Many journals, as a result of the discussions about reproducibility of scientific results, encourage authors to submit a model together with a publication, and they advise authors to publish model code in  open repositories using  standard formats. 
Journals like FEBS or PLoS specifically ask for SBML files, other journals make a more generic statement. 
However, all major systems biology journals accept SBML files for submission of model code.
Examples are available on the SBML website at \url{http://sbml.org}.
...
Nevertheless, specific challenges remain, as discussed below. These will need to be tackled to ensure SBML persistence and future developments.
\newpage
...
Besides these scientific and technical challenges, important social and cultural challenges exist for a standard like SBML. These include support by funding agencies, be it financial to enable continuous support and development of SBML and for support in SBML in software, or political, by mandating that funded projects provide developed models in standard formats. Support by journals and reviewers is another critical issue which should require that modeling studies provide the model in a standard format, or when not feasible that the model code is provided. Many in the field are frustrated by the lack of availability and reproducibility of published models. Another important social challenge is education and outreach, especially to early-carrier scientists, PhDs, masters and bachelors.
A key factor for the success of SBML is the rich software ecosystem, providing easy access to SBML, \eg via GUI based software or simplified text-based SBML compatible formats like antimony. At the same time this software ecosystem is also the major reason of frustration with SBML. Problems with SBML are mostly a failure of specific software, not a failure of the underlying SBML standard. Improving this software layer requires a continuous investment in tool development.

% ======================================================================
\section*{Conclusion}
% ======================================================================

SBML and associated software libraries and tools have been instrumental in the growth of systems biology for nearly twenty years.  As modeling and simulation grew in popularity as a way to gain insight into biological phenomena, SBML allowed researchers to exchange and (re)use new models in an open, well-supported, interoperable format. SBML has made possible much of the research pursued by the authors of this article, and also helped us to structure our thoughts about our models and the biology they represent.  Today, scientists can build, manipulate, annotate, store, reuse, publish, and connect models to each other and to basic data sources.  In effect, SBML has turned models into a kind of data and moved modeling in biology from an art to an exercise in engineering.

...
Detailed information on software, algorithms, and parameters used in a model are necessary to meet the reproducibility challenge \citep{waltemath2016modeling}. Sophisticated  annotation schemes \citep{Neal2019harmonizing}, mechanisms to track the provenance of SBML-encoded models \citep{scharm2018evolution}, and training in reproducible modeling (\url{https://reproduciblebiomodels.org/}) lead to the better reproducibility of scientific modeling results, another important contribution of the SBML community to the scientific field. 
...
Though this modular approach has benefits, it is not without potential pitfalls.  The main risks are fragmentation of the community, and incompatibility of packages due to complex feature dependencies.  The SBML community addressed the former by maintaining communications between package developers.  
The community structuring described below aims to enable such interactions. 
As for the latter, API libraries (see Box~\ref{box:software}) can handle \emph{some} combinations of packages and hide some of the complexity.  Still, there remain some combinations of packages that are not fully understood, and it remains for future work to define how (if ever) they can be combined for use in a single model.
...
A wider impact of SBML as a \emph{de facto} standard has been the support of publishers and funding agencies.  Many journals, aware of the challenges surrounding the reproducibility of scientific results, encourage authors to make their actual models available to readers.  Some journals, such as the BMC and FEBS journals, explicitly encourage authors to submit SBML files as supporting material for research where it is relevant\footnote{\url{https://www.biomedcentral.com/getpublished/writing-resources/additional-files}}\footnote{\url{https://onlinelibrary.wiley.com/page/journal/17424658/homepage/ForAuthors.html}}; others, such as Biophysical Journal \citep{nickerson2017introducing}, recommend authors deposit models in repositories such as BioModels Database~\citep{chelliah2014biomodels}, which---by virtue of those repositories' requirement to use a standard format such as SBML---is an indirect encouragement to use SBML when possible.  However, even if submission in a particular format is not mentioned (many journals do not require submission in \emph{any} specific model format), all major systems biology journals do accept SBML files for submission of model code~\footnote{\url{http://sbml.org/Documents/Publications_known_to_accept_submissions_in_SBML_format}}.  Many funding agencies now have policies related to data-sharing, which some extend to research producing computational models, and they also request models to be in standard formats~\footnote{See, for example, \url{https://grants.nih.gov/grants/guide/pa-files/par-08-023.html}}.  

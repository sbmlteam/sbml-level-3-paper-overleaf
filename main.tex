...
The initial release of SBML Level~3 in %2010~\citep{Hucka2015}
\citeyear{Hucka2010a} and its revisions \citep{Hucka2010a,Hucka2015,Hucka2018b}
have provided a new foundation to enable the exchange of a greater variety of models in various domains of biology (Figure~\ref{level-3-diagram}).
...
Besides these scientific and technical challenges, urging social and cultural challenges exist for a standard like SBML.  These include support by funding agencies, be it financial to enable continuous support and development of SBML and for support in SBML in software, or political, by mandating that funded projects provide developed models in standard formats.  Support by journals and reviewers is another critical issue that should require that modeling studies provide the model in a standard format, or when not feasible to provide the model code.  Many in the field are frustrated by the lack of availability and reproducibility of published models.  Another important social challenge is education and outreach, especially to early-carrier scientists, PhDs, masters, and bachelors.
A pivotal factor for the success of SBML is the extensive software ecosystem, providing easy access to SBML, \eg via GUI-based software or simplified text-based SBML compatible formats such as Antimony.  At the same time, this software ecosystem is also the major reason of frustration with SBML.  Problems with SBML are mostly a failure of specific software, not a failure of the underlying SBML standard.  Improving this software layer requires a continuous investment in tool development.
